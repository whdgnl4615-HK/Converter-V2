...
export default function ConverterPage({ module: moduleKey }) {
  ...
  function applyAllCleanses() {
    ...
    setAiCleanseMsg(`✅ ${cleanseResults.length}개 {T.converter.aiCleanseApply} 완료`)
    if (step === 3) rebuildPreview(newMapping)
  }
        ...
        <div className="flex gap-1">
          {[
            ...
            <div key={n} className="step-pill"
              style={{
                background: step === n ? 'var(--accent-light)' : step > n ? 'var(--green-bg)' : 'var(--s2)',
                color: step === n ? 'var(--accent)' : step > n ? 'var(--green)' : 'var(--text4)',
                borderColor: step === n ? 'var(--accent)' : step > n ? 'var(--green-border)' : 'var(--border)',
              }}>
              {step > n && <span style={{fontSize:10}}>✓</span>}
              <span>{label}</span>
            </div>
          ))}
        </div>
            ...
            <div>
              ...
              <div
                ...
                style={{
                  ...
                  background: dragging ? 'var(--accent-light)' : 'var(--s1)',
                  padding: '80px 40px',
                  minHeight: '320px',
                }}
            ...
            <div className="flex flex-col h-full" style={{minHeight:0}}>
              ...
              <div className="flex-shrink-0 mb-3">
                  ...
                  <div className="flex flex-col gap-2 flex-1">
                    ...
                    <div className="flex items-center gap-2">
                      <button onClick={handleAICleanse} disabled={aiCleansing}
                        ...
                        {aiCleansing
                          ? <><span style={{display:'inline-block',animation:'spin 1s linear infinite'}}>⟳</span> {T.converter.aiAnalyzing}</>
                          : <>{T.converter.aiSuggest}</>
                        }
                      </button>
                      {aiCleanseMsg && (
                    ...
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        placeholder="{T.converter.aiChatPlaceholder}"
                ...
                <div className="grid grid-cols-3 gap-2 mb-2">
                  {[
                    [evdData.length, T.converter.records],
                  ...
                  <div className="rounded-xl mb-2 overflow-hidden" style={{border:'1px solid var(--accent)',background:'rgba(79,99,210,0.04)'}}>
                    <div className="px-4 py-2 flex items-center justify-between"
                      ...
                      <span className="text-xs mono font-bold" style={{color:'var(--accent)'}}>
                        {lang==='ko' ? `✨ AI 클렌징 제안 — ${cleanseResults.length}개` : `✨ AI Cleanse — ${cleanseResults.length} suggestions`}
                      </span>
                      <div className="flex gap-2">
                        <button onClick={applyAllCleanses}
                          className="px-3 py-1 rounded-lg text-xs mono font-bold"
                          style={{background:'var(--accent)',color:'white'}}>{T.converter.aiCleanseApply}</button>
                ...
                <div className="flex-1 flex flex-col rounded-xl overflow-hidden" style={{border:'1px solid var(--border)',minHeight:0}}>
                  <div className="flex items-center justify-between px-3 py-2 flex-shrink-0"
                    ...
                    <span className="text-xs mono font-bold uppercase" style={{color:'var(--text3)',letterSpacing:'1px'}}>
                      📄 {lang==='ko' ? T.converter.sourceData : 'Source Data'}
                    </span>
                ...
                <div className="flex-1 flex flex-col rounded-xl overflow-hidden" style={{border:'1px solid var(--green)',minHeight:0}}>
                  <div className="flex items-center justify-between px-3 py-2 flex-shrink-0"
                    ...
                    <span className="text-xs mono font-bold uppercase" style={{color:'var(--green)',letterSpacing:'1px'}}>
                      ✅ {lang==='ko' ? T.converter.n41Output : 'N41 Output'}
                    </span>
