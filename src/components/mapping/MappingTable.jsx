    ...
    <div>
      ...
      <div className="flex items-center gap-3 mb-3 px-1">
        <button onClick={handleAISuggest} disabled={aiLoading || !sourceColumns.length}
          ...
          {aiLoading ? (
            <><span style={{animation:'spin 1s linear infinite',display:'inline-block'}}>⟳</span> {T.converter.aiAnalyzing}</>
          ) : (
            <>{T.converter.aiMappingSuggest}</>
          )}
        </button>
        ...
        {hasSuggestions && (
          <button onClick={applyAllSuggestions}
            ...
            {T.converter.aiApplyAll.replace("{n}", Object.keys(aiSuggestions).length)}
          </button>
        )}
            ...
            return (
              <div key={col} className="grid items-start px-4 py-2"
                ...
                <div>
                  ...
                  {isApplied && (
                    <div className="mt-1 text-xs mono" style={{color:'var(--green)'}}>{T.converter.aiApplied}</div>
                  )}
                </div>
