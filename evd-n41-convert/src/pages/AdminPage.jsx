...
export default function AdminPage() {
  ...
  const ROLE_BADGE = {
    admin:   { bg: 'rgba(247,163,92,0.1)',  color: 'var(--orange)', label: 'admin' },
    user:    { bg: 'rgba(61,214,140,0.08)', color: 'var(--green)',  label: 'user' },
              ...
              {active.length === 0 ? (
                ...
                return (
                  <div key={u.id} className="flex items-center justify-between px-4 py-3"
                    style={{borderBottom: i<active.length-1 ? '1px solid rgba(44,44,66,0.5)' : 'none',background:'var(--s1)'}}>
                    <div>
          ...
          {tab === 'pending' && (
            <div className="rounded-xl overflow-hidden" style={{border:'1px solid var(--border)'}}>
              {pending.length === 0 ? (
                ...
                <div key={u.id} className="flex items-center justify-between px-4 py-3"
                  style={{borderBottom: i<pending.length-1?'1px solid rgba(44,44,66,0.5)':'none',background:'var(--s1)'}}>
                  <div>
                    <div className="text-sm" style={{color:'var(--text)'}}>{u.email}</div>
                    <div className="text-xs mono mt-0.5" style={{color:'var(--text3)'}}>
                      {new Date(u.created_at).toLocaleDateString()}
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => updateRole(u.id, 'user')}
                      ...
                      style={{background:'rgba(61,214,140,0.1)',color:'var(--green)',border:'1px solid rgba(61,214,140,0.2)'}}>
                      {T.admin.approve}
                    </button>
                    <button onClick={() => updateRole(u.id, 'rejected')}
                      className="text-xs mono px-3 py-1.5 rounded"
                      style={{background:'rgba(247,108,108,0.08)',color:'var(--red)',border:'1px solid rgba(247,108,108,0.2)'}}>
                      {T.admin.reject}
                    </button>
                  </div>
                </div>
            ...
            <div className="rounded-xl overflow-hidden" style={{border:'1px solid var(--border)'}}>
              {invitations.length === 0 ? (
                ...
                <div key={inv.id} className="flex items-center justify-between px-4 py-3"
                  style={{borderBottom: i<invitations.length-1?'1px solid rgba(44,44,66,0.5)':'none',background:'var(--s1)'}}>
                  <div>
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-xs mono px-2 py-0.5 rounded"
                      style={inv.status==='accepted'
                        ...
                        : {background:'rgba(92,190,247,0.08)',color:'var(--blue)'}}>
                      {inv.status}
                    </span>
                    {inv.status === 'pending' && (
