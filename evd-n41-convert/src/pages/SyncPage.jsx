...
const PLATFORMS = {
  fashiongo: {
    ...
    icon: '🛍️',
    color: 'var(--blue)',
    colorBg: 'rgba(92,190,247,0.08)',
    colorBorder: 'rgba(92,190,247,0.25)',
...
function StatusBadge({ status }) {
  const cfg = {
    ok:      { bg: 'rgba(61,214,140,0.1)',  color: 'var(--green)',  label: '✓ OK' },
    diff:    { bg: 'rgba(247,163,92,0.1)',  color: 'var(--orange)', label: '⚡ DIFF' },
    missing: { bg: 'rgba(247,108,108,0.1)', color: 'var(--red)',    label: '✕ N41 없음' },
    warn:    { bg: 'rgba(92,190,247,0.1)',  color: 'var(--blue)',   label: '⚠ WARN' },
  }
  ...
  return (
    <div className="flex flex-col gap-0.5">
      <div className="flex items-center gap-1 flex-wrap">
        <span className="text-xs mono px-1.5 py-0.5 rounded" style={{ background: 'rgba(247,108,108,0.12)', color: 'var(--red)' }}>
          {diff.shopifyVal ?? diff.fgVal ?? '—'}
        </span>
        <span style={{ color: 'var(--text3)', fontSize: 10 }}>→</span>
        <span className="text-xs mono px-1.5 py-0.5 rounded" style={{ background: 'rgba(61,214,140,0.12)', color: 'var(--green)' }}>
          {diff.n41Val}
        </span>
      </div>
    ...
    <div className="flex flex-col h-screen overflow-hidden">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="w-72 flex-shrink-0 overflow-y-auto p-4 flex flex-col gap-4"
          ...
          {error && (
            <div className="text-xs mono p-3 rounded-lg" style={{ background: 'rgba(247,108,108,0.1)', color: 'var(--red)', border: '1px solid rgba(247,108,108,0.2)' }}>
              {error}
            </div>
          )}
                ...
                <table className="text-xs w-full" style={{ borderCollapse: 'collapse' }}>
                  ...
                  <tbody>
                    {filtered.length === 0 ? (
                      ...
                      <tr key={r._idx}
                        style={{
                          borderBottom: '1px solid rgba(44,44,66,0.4)',
                          background: r.accepted ? 'rgba(61,214,140,0.04)' : rowI % 2 === 0 ? 'var(--s1)' : 'transparent',
                        }}>
